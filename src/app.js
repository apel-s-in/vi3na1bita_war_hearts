...
const openRankedAuthGate = ({ title = 'Рейтинговый бой', text = 'Войдите через Яндекс. Сетевые бои всегда рейтинговые и требуют ставку 100 ♦.', loginText = 'Войти через Яндекс', onAuthed } = {}) => {
...
const registerShotStats = (side, result) => {
  ...
  if (side === 'player') {
    ...
    if (result === 'sunk') {
      ...
      stats.playerBestSunkStreak = Math.max(Number(stats.playerBestSunkStreak || 0), stats.playerSunkStreak);
    } else if (result === 'miss') {
  ...
  if (result === 'sunk') {
    ...
    stats.opponentBestSunkStreak = Math.max(Number(stats.opponentBestSunkStreak || 0), stats.opponentSunkStreak);
  } else if (result === 'miss') {
...
const actions = {
  ...
  `;
    ...
    try {
      const module = await import('/Games/common/friends-embed.js?v=9.1.9');
  ...
  startLanGameFlow() {
    ...
    openRankedAuthGate({ title: '📶 Рейтинговый бой по Wi‑Fi/LAN', text: 'Для боя с реальным игроком нужен вход через Яндекс и 100 доступных Осколков.', loginText: 'Войти и продолжить', onAuthed: open });
  },
  openMenu() {
    clearTimeout(computerTimer);
    clearTimeout(playerAutoTimer);
...
const createLanRoom = async () => {
  ...
  try {
    ...
    state.invite = { id: res.roomId, roomId: res.roomId, roomSecret: res.roomSecret, code: res.code, isLan: true, localOnly: true, ranked: true, matchMode: 'ranked', expiresAt: res.expiresAt || Date.now() + 300000 };
    state.network.active = true;
    state.network.connected = false;
    state.network.status = 'waiting';
    ...
    state.network.lastEventAt = Date.now();
    addSystemMessage('Создан рейтинговый бой за 100 ♦. Результат будет принят только после серверной fair-play проверки.');
    setScreen('invite');
  } catch (error) {
    toast(error.message === 'lan_code_register_failed' ? 'Не удалось зарегистрировать код комнаты' : `Ошибка: ${error.message}`);
  }
};
...
const bind = () => {
      ...
      state.network.text =
        {
          ...
          'ice gathering slow': 'Сбор ICE занял больше времени. Отправляем поздние кандидаты отдельно...',
          ...
          'signaling degraded': 'P2P работает напрямую. Вспомогательный signaling временно недоступен.',
          'presence degraded': 'P2P работает. Сетевой статус игрока временно не обновляется.',
  ...
  session.onConnect = peer => {
    const ranked = !!(session.room?.ranked ?? session.bridge?.ranked);
    const localOnly = !!(session.room?.localOnly ?? session.bridge?.forceLocalOnly);
    state.opponent = { id: peer?.id || 'network-peer', name: peer?.name || 'Соперник', title: localOnly ? (ranked ? 'LAN · рейтинг' : 'LAN · гость') : 'Сетевая дуэль', type: 'network' };
    state.network.active = true;
...
sessionReady = waitForHostBridge()
  .then(async bridgeReady => {
    ...
    if (inviteFriendId && !isLaunchCancelled()) {
      ...
      try {
        ...
        await actions.inviteFriend(inviteFriendId, profile?.displayName || 'Друг');
      } catch (error) {
  ...
  .catch(error => {
    session.lastError = String(error?.message || 'network_bridge_init_failed');
    state.network.connected = false;
    state.network.status = 'error';
