...
const showLanModeChoice = () => {
  ...
  overlay.innerHTML = `
    <div class="wh-modal-box">
      <h3 class="wh-modal-title">📡 Создание LAN-комнаты</h3>
      <p class="wh-modal-text" style="margin-bottom:16px">
        Выберите тип боя. Гость получит этот тип автоматически по коду.
      </p>
      <div class="wh-modal-actions" style="flex-direction:column;gap:10px">
        <button class="wh-btn" type="button" id="lan-ranked-btn" ${!isAuthed ? 'disabled' : ''} style="background:linear-gradient(135deg,#ff9800,#f57c00);${!isAuthed ? 'opacity:.5' : ''}">
          🏆 Рейтинговый бой
          <br><small style="font-size:10px;opacity:.8">${isAuthed ? 'учитывается в таблице лидеров' : 'требуется вход через Яндекс'}</small>
        </button>
        <button class="wh-btn secondary" type="button" id="lan-casual-btn" style="background:linear-gradient(135deg,#7c4dff,#4daaff)">
          👤 Гостевой бой
          <br><small style="font-size:10px;opacity:.8">без рейтинга и статистики</small>
        </button>
...
const joinLanByCode = async code => {
  ...
  try {
    ...
    const res = await session.joinLanRoom(code, { forceLocalOnly: true });
