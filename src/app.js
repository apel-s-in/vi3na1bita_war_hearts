...
overlay.innerHTML = `
<div class="wh-modal-box">
<h3 class="wh-modal-title">📶 Игра по одной Wi-Fi</h3>
<p class="wh-modal-text" style="margin-bottom:16px">
Прямое P2P-соединение с минимальной задержкой.<br>
Убедитесь, что оба устройства подключены к одной сети.
</p>
...
// ═══════════════════════════════════════════════════════════════
const showLanModeChoice = (role) => {
const isAuthed = !!state.snapshot?.user?.yandexLinked;
const overlay = document.createElement('div');
overlay.className = 'wh-modal-overlay';
overlay.innerHTML = `
<div class="wh-modal-box">
<h3 class="wh-modal-title">${role === 'host' ? '📡 Создание комнаты' : '🔗 Подключение'}</h3>
<p class="wh-modal-text" style="margin-bottom:16px">
Выберите тип боя:
</p>
<div class="wh-modal-actions" style="flex-direction:column;gap:10px">
<button class="wh-btn" type="button" id="lan-ranked-btn" style="background:linear-gradient(135deg,#ff9800,#f57c00)" ${!isAuthed ? 'disabled style="opacity:.5"' : ''}>
🏆 Рейтинговый бой
${!isAuthed ? '<br><small style="font-size:10px;opacity:.7">Требуется авторизация Яндекса</small>' : '<br><small style="font-size:10px;opacity:.8">+осколки за победу, в таблицу лидеров</small>'}
</button>
<button class="wh-btn secondary" type="button" id="lan-casual-btn" style="background:linear-gradient(135deg,#7c4dff,#4daaff)">
👤 Гостевой бой
<br><small style="font-size:10px;opacity:.8">Без рейтинга и осколков, для удовольствия</small>
</button>
<button class="wh-btn secondary" type="button" id="lan-back-btn" style="background:transparent;border:1px solid rgba(255,255,255,.2)">Назад</button>
</div>
</div>
`;
document.body.appendChild(overlay);
overlay.querySelector('#lan-back-btn').onclick = () => {
overlay.remove();
actions.startLanGameFlow();
};
overlay.querySelector('#lan-casual-btn').onclick = () => {
overlay.remove();
state.network.ranked = false;
if (role === 'host') {
createLanRoom(false);
} else {
joinLanByCode(state.network.pendingLanCode, false);
}
};
const rankedBtn = overlay.querySelector('#lan-ranked-btn');
if (rankedBtn && !rankedBtn.disabled) {
rankedBtn.onclick = () => {
overlay.remove();
...
joinLanByCode(state.network.pendingLanCode, true);
}
};
}
};
...
<input type="text" id="lan-code-input" maxlength="6" placeholder="A7B9C2"
style="width:100%;padding:14px;font-size:24px;text-align:center;font-weight:900;letter-spacing:4px;text-transform:uppercase;border-radius:12px;border:1px solid rgba(255,255,255,.15);background:rgba(0,0,0,.3);color:#fff;outline:none;margin-bottom:16px"
...
state.network.connected = false;
state.network.status = 'waiting';
state.network.peerName = 'Ожидание гостя...';
state.network.text = 'Комната создана. Ожидаем подключения...';
...
const joinLanByCode = async (code, ranked) => {
toast('Подключаемся по коду...');
try {
await sessionReady;
...
state.network.text = 'Устанавливаем P2P-соединение...';
state.network.ranked = ranked;
setScreen('invite');
} catch (e) {
toast(e.message === 'lan_room_not_found' ? 'Комната не найдена или истекла' : `Ошибка: ${e.message}`);
}
};
