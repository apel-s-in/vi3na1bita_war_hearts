...
postToHost('GC_READY');
const session = new WarHeartsSession({ gameId: 'war_hearts', player: state.player });
let computerTimer = 0;
...
const openRankedAuthGate = ({ title = 'Рейтинговый бой', text = 'Войдите через Яндекс, чтобы сыграть рейтингово.', loginText = 'Войти через Яндекс', guestText = 'Сыграть гостевой бой', onAuthed, onGuest } = {}) => {
...
const createLanRoom = async ranked => {
  ...
  try {
    ...
    state.network.text = ranked ? 'P2P-комната создана. Назовите код другу.' : 'Гостевая P2P-комната создана. Назовите код другу.';
    state.network.ranked = !!ranked;
    state.network.localOnly = false;
    state.network.matchMode = ranked ? 'ranked' : 'casual';
    state.network.lastEventAt = Date.now();
    addSystemMessage(ranked ? 'Создана рейтинговая P2P-комната. Результат будет учтён после fair-play проверки.' : 'Создана гостевая P2P-комната. Результат не попадёт в статистику.');
    setScreen('invite');
  } catch (e) {
    toast(e.message === 'lan_code_register_failed' ? 'Не удалось зарегистрировать LAN-код' : `Ошибка: ${e.message}`);
  }
};
...
const connectLanRoom = async (code, rankedOverride = null) => {
  ...
  state.network.text = ranked ? 'Код принят: рейтинговый P2P-бой. Устанавливаем соединение...' : 'Код принят: гостевой P2P-бой. Устанавливаем соединение...';
  state.network.ranked = ranked;
  state.network.localOnly = false;
  state.network.matchMode = ranked ? 'ranked' : 'casual';
  state.network.lastEventAt = Date.now();
  addSystemMessage(ranked ? 'Подключение к рейтинговому P2P-бою. Результат будет учтён после проверки.' : 'Подключение к гостевому P2P-бою. Результат не попадёт в статистику.');
  setScreen('invite');
};
const render = () => {
  ...
  .then(async () => {
    ...
    if (inviteFriendId && !isLaunchCancelled()) {
      ...
      try {
        ...
        actions.inviteFriend(inviteFriendId, profile?.displayName || 'Друг');
      } catch (e) {
