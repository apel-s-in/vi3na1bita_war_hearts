...
export const summarizeProgress = ({ stats = {}, history = [] } = {}) => {
  ...
  if (!rows.length) {
    ...
    return {
      ...
      bestSunkStreak: num(stats.playerBestSunkStreak),
      bestOpponentSunkStreak: num(stats.opponentBestSunkStreak),
  ...
  return {
    ...
    bestSunkStreak: Math.max(0, ...rows.map(row => num(row.playerBestSunkStreak))),
    bestOpponentSunkStreak: Math.max(0, ...rows.map(row => num(row.opponentBestSunkStreak))),
