...
export const createNetworkCombat = ({
  ...
  const setNetworkStatus = (text, status = 'info') => {
    state.network.active = state.opponent?.type === 'network';
    state.network.status = status;
    state.network.text = text;
    state.network.connected = status !== 'offline' && status !== 'error';
  ...
  const openReadyModal = () => {
    clearModal('.wh-network-ready-overlay');

    const overlay = document.createElement('div');
    overlay.className = 'wh-modal-overlay wh-network-ready-overlay';
    overlay.innerHTML = `
      <div class="wh-modal-box">
        <h3 class="wh-modal-title">Сетевой бой</h3>
        <p class="wh-modal-text">
          Расставьте корабли на поле. Когда будете готовы — нажмите кнопку.
          После готовности начнётся синхронизация с соперником.
        </p>
        <div class="wh-modal-actions">
          <button class="wh-btn secondary" type="button" id="wh-network-ready-later">Позже</button>
          <button class="wh-btn" type="button" id="wh-network-ready-now">Готов к бою</button>
        </div>
      </div>
    `;
    ...
    overlay.querySelector('#wh-network-ready-later')?.addEventListener('click', () => {
      overlay.remove();
      setNetworkStatus('Расставьте корабли. Когда будете готовы — подтвердите готовность.', 'setup');
    });

    overlay.querySelector('#wh-network-ready-now')?.addEventListener('click', async () => {
      overlay.remove();
      await markReady();
    });
  };
  ...
  const receiveBoardReveal = async msg => {
    ...
    if (layoutCheck.ok && commitCheck.ok) {
      applyRevealToBoard(state.enemyBoard, reveal);
      state.fairPlay.note = 'BOARD_REVEAL соперника проверен: commit совпал, расстановка корректна.';
      addSystemMessage('Проверка соперника: честность OK.');
      setNetworkStatus('Reveal соперника проверен. Честность OK.', 'ready');
    } else {
      state.fairPlay.note = `Проблема проверки: ${layoutCheck.reason || commitCheck.reason}.`;
      addSystemMessage(`Проверка соперника не пройдена: ${state.fairPlay.note}`);
      setNetworkStatus('Reveal соперника не прошёл проверку.', 'error');
    }
