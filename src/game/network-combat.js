...
export const createNetworkCombat = ({
  ...
  const maybeStartRps = () => {
    ...
    state.network.ranked = true;
    state.network.matchMode = 'ranked';
    setNetworkStatus('Оба игрока готовы. Рейтинговый бой за 100 ♦ зафиксирован. Разыгрываем первый ход.', 'ready');
    addSystemMessage('Оба игрока готовы. Ставки заблокированы, первый ход определяет серверный RPS.');
    openNetworkRpsModal();
    scheduleSaveMatchDraft();
  };
  ...
  const handleGameData = msg => {
    ...
    switch (msg.type) {
      ...
      case MessageType.MATCH_MODE:
        ...
        state.network.ranked = true;
        state.network.matchMode = 'ranked';
        addSystemMessage('Режим боя подтверждён: рейтинговый, ставка 100 ♦.');
        setNetworkStatus('Режим боя: рейтинговый · 100 ♦.', 'setup');
        scheduleSaveMatchDraft();
        break;
      default:
