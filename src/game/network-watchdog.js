...
const now = () => Date.now();
export const createNetworkWatchdogState = () => ({ active: false, lastPeerAt: 0, lastPingAt: 0, lastPongAt: 0, lastWarningAt: 0, softTimeoutMs: 20000, hardTimeoutMs: 45000, pendingTimeoutMs: 16000, warning: false, note: '' });
...
export const createNetworkWatchdog = ({ state, session, render, addSystemMessage, scheduleSaveMatchDraft }) => {
  let timer = 0;
  let paused = false;
  const touchPeer = () => {
  ...
  const sendPingIfNeeded = () => {
    ...
    wd.lastPingAt = stamp;
    session.sendGame(MessageType.PING, { matchId: state.matchStats?.matchId || '', phase: state.phase, awaitingShotResult: !!state.network?.awaitingShotResult, awaitingReveal: !!state.network?.awaitingReveal, hidden: !!document.hidden });
  };
  const checkPending = () => {
    if (!state.network?.active || !state.network?.connected) return;
    const wd = ensureNetworkWatchdogState(state);
  ...
  return { start, stop, pause, resume, tick, touchPeer, markPong, warn, clearWarning };
};
