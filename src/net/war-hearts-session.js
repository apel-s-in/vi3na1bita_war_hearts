...
export class WarHeartsSession {
  ...
  async createLanRoom({ ranked = false, forceLocalOnly = false } = {}) {
    ...
    for (let i = 0; i < 3 && !registered?.ok; i++) {
      ...
      registered = await this.bridge.registerLanCode?.(code, room.roomId, room.roomSecret, ranked, forceLocalOnly).catch(() => null);
    }
    if (!registered?.ok) throw new Error('lan_code_register_failed');
    this.room = { roomId: room.roomId, roomSecret: room.roomSecret, code, ranked: !!ranked, localOnly: !!forceLocalOnly, matchMode: ranked ? 'ranked' : 'casual', joinUrl: room.joinUrl };
    return { roomId: room.roomId, roomSecret: room.roomSecret, code, ranked: !!ranked, localOnly: !!forceLocalOnly, matchMode: ranked ? 'ranked' : 'casual', joinUrl: room.joinUrl, expiresAt: registered.expiresAt };
  }
  ...
  async joinLanRoom(code, { forceLocalOnly = false, rankedOverride = null } = {}) {
    ...
    this.room = { roomId: roomInfo.roomId, roomSecret: roomInfo.roomSecret, code: roomInfo.code, ranked, localOnly, matchMode: ranked ? 'ranked' : 'casual', expiresAt: roomInfo.expiresAt || 0 };
    await this.bridge.connectAsGuest({ roomId: roomInfo.roomId, roomSecret: roomInfo.roomSecret, forceLocalOnly: localOnly, ranked, rankedOverride });
    this.room = { ...this.room, ranked: !!this.bridge.ranked, localOnly: !!this.bridge.forceLocalOnly, matchMode: this.bridge.ranked ? 'ranked' : 'casual' };
    return this.room;
  }
