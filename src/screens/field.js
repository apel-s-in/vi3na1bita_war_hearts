...
const loadPresets = state => {
  const saved =
    state.snapshot?.gameData?.war_hearts_presets ||
    state.snapshot?.gameData?.presets;
...
const savePresets = (list, state) => {
  memoryPresets = Array.isArray(list)
    ? list.slice(0, 4)
    : [];

  if (state.snapshot) {
    state.snapshot.gameData = {
      ...(state.snapshot.gameData || {}),
      war_hearts_presets: memoryPresets
    };
  }
  ...
  if (window.parent !== window) {
    window.parent.postMessage({
      kind: 'vitrina:game',
      bridgeId: window.__GC_BRIDGE_ID || '',
      type: 'GC_SAVE_DATA',
      payload: {
        gameId: 'war_hearts',
        key: 'presets',
...
const rankBadge =
  '<span style="display:inline-block;padding:3px 10px;border-radius:999px;background:rgba(255,152,0,.2);border:1px solid rgba(255,152,0,.4);color:#ffb74d;font-size:11px;font-weight:900;margin-left:8px">🏆 РЕЙТИНГ · 100 ♦</span>';
